class Autocomplete {
  ...
  getMatches(text) {
    ...
    const arr = []; //накапливаем за пределами
...
/* Выпадающий список - тут есть ошибки, давайте разбираться: у нас есть опции this.input.options, по нему необходимо пройтись циклом, и в случае если у итерируемой option есть совпадение с аргументом text добавляем в буферный массив объект по примеру
{
text: 'Содержимое <option>',
value: 'Содержимое атрибута value'
}
4) Вообще вывод текста в return правильно вывожу? - да, только данные формируете некорректно.
